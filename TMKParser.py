...
def err_tmk(row, indexes):
    ...
    for indx in indexes:
        cell = row[indx].value
        if indx == -1 or cell is None:
            continue
        for def_err in errs.keys():
            if def_err in cell and errs[def_err] not in err:
                err += errs[def_err]
...
class TMKParser:
    ...
    def get_columns(self, row, heat_cols={'Время': -1, 't1': -1, 't2': -1,'V1': -1,'M1': -1,'V2': -1,'M2': -1, 'Q': -1, 'Tн': -1,'ВОС': -1, 'НС': -1, 'НС1': -1, 'НС2': -1}):
        ind = 0
        #rep_type = '1'
        for cell in row:
            if cell.value == None:
            ...
            for key in heat_cols.keys():
                if key in cell.value and 'G1-G2' not in cell.value and 'V1-V2' not in cell.value and 'Q3' not in cell.value:
                    heat_cols[key] = ind
            ...
            if 'G4' in cell.value and heat_cols['M2'] == -1 and 'G3-G4' not in cell.value:
                heat_cols['M2'] = ind
            if ('Q,' in str(cell.value) or 'Q1,' in str(cell.value) or 'Q2,' in str(cell.value)) and heat_cols['Q'] == -1:
                print('Q: ', ind)
                heat_cols['Q'] = ind
            if ('Tн' in cell.value or 'Время' in cell.value or 'Tраб сис' in cell.value or 'T1' in cell.value or 'T2' in cell.value) and heat_cols['Tн'] == -1:
                heat_cols['Tн'] = ind
            if ('Tр,' in cell.value or 'Tраб,' in str(cell.value)) and heat_cols['ВОС'] == -1:
                heat_cols['ВОС'] = ind
    ...
    def time_with_minutes(self, t):
        if t.value == None:
            return 0
        else:
            return round(float(str(t.value).split(':')[0]) + 0.0167 * float(str(t.value).split(':')[1]), 2)
    ...
    def build_xls(self, file, rep_type, start_col=1):
        ...
        data_indexes = {'Время': -1, 't1': -1, 't2': -1,'V1': -1,'M1': -1,'V2': -1,'M2': -1, 'Q': -1, 'Tн': -1,'ВОС': -1, 'НС': -1, 'НС1': -1, 'НС2': -1}
        ...
        for row in file[1].iter_rows(min_col=start_col):
            ...
            err = err_tmk(row, [data_indexes['НС'], data_indexes['НС1'], data_indexes['НС2']])
        ...
        while file[1][sum_table_index][0].value != 'Дата':
            sum_table_index += 1
        data_indexes = self.get_columns(file[1][sum_table_index], {'Время': -1, 't1': -1, 't2': -1,'V1': -1,'M1': -1,'V2': -1,'M2': -1, 'Q': -1, 'Tн': -1,'ВОС': -1, 'НС': -1, 'НС1': -1, 'НС2': -1})
        ...
        if float_time_finnaly('ВОС') != ' - ' and data_indexes['ВОС'] != -1:
            vos_start = self.time_with_minutes(summary_data[data_indexes['ВОС']]) - vnr_start
            vos = self.time_with_minutes(file[1][sum_table_index + 2][data_indexes['ВОС']]) - vnr
        ...
        if vos_start > 0:
            ...
            ws[vos_col + str(sec_row)] = str(round(0, 2)).replace('.', ',')
        if vos > 0:
            ws[vos_col + str(sec_row + 1)] = str(round(vos, 2)).replace('.', ',')
        else:
            ws[vos_col + str(sec_row + 1)] = str(round(0, 2)).replace('.', ',')
